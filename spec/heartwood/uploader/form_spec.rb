...
RSpec.describe Heartwood::Icons::Form do

  let(:form) { Heartwood::Icons::Form.new }
  ...
  describe '#form_options' do
    it 'returns key/value pairs for form options' do
      ...
      expect(opts[:id]).to eq('heartwood-icons')
  ...
  describe '#validate_options!' do
    it 'throws an error if aws keys are nil' do
      Heartwood::Icons.configuration.aws_access_key_id = nil
      expect { Heartwood::Icons::Form.new }.to raise_error(ArgumentError)

      Heartwood::Icons.configuration.aws_access_key_id = 'aws_access_key_id'
      Heartwood::Icons.configuration.aws_bucket = nil
      expect { Heartwood::Icons::Form.new }.to raise_error(ArgumentError)

      Heartwood::Icons.configuration.aws_access_key_id = 'aws_access_key_id'
      Heartwood::Icons.configuration.aws_bucket = 'aws_bucket'
      Heartwood::Icons.configuration.aws_secret_access_key = nil
      expect { Heartwood::Icons::Form.new }.to raise_error(ArgumentError)
    end
  end
  ...
  describe '#default_options, #initialize, #method_missing' do
    it 'has a set of default options, available as attributes' do
      ...
      expect(form.form_id).to eq('heartwood-icons')
      ...
      expect(form.upload_field_id).to eq('heartwood-icons-file')
      expect(form.url_field_class).to eq('heartwood-icons-url')
      expect(form.url_field_name).to eq(nil)
    end
    it 'will override options on init' do
      form = Heartwood::Icons::Form.new(
        acl: '[custom]_acl',
